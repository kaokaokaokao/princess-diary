...
def driver_open(driver, key_word):
    ...
    time.sleep(2)
    driver.find_element_by_css_selector('#username').send_keys('0211113089')
    time.sleep(3)
    driver.find_element_by_css_selector('#password').send_keys('33028352@qq.Com')
    time.sleep(2)
...
def get_author_info(skey, code):
    ...
    if div:
        ...
        # print(major[0])
        college = college.split('曾')[0]
        if major[0] == '研':
            pass
        else:
            major = h3[2].get_text().strip()
...
if __name__ == '__main__':
    ...
    soup = driver_open(driver, '近代中国')  # 搜索知识图谱
    papers = []     # 用于保存爬取到的论文
    # 将爬取到的论文数据放入papers中
